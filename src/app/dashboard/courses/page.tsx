...
export default function CoursesPage() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (tRes.ok) {
          ...
          setTermStatuses([
              { term: 1, is_locked: false, is_published: false },
              { term: 2, is_locked: false, is_published: false },
              { term: 3, is_locked: false, is_published: false },
          ]);
      }
  ...
  const handleLockTerm = async (termId: number) => {
    if (!confirm(`Are you absolutely sure? Locking Term ${termId} is IRREVERSIBLE. This will trigger automated result emails to all students and lock all entries for this term.`)) return;
